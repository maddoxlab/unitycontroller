...
# %% Make a trial info matrix to help Unity with stimulus parsing
stim_info = np.zeros((n_trl, 3)) # cols: spatial cong., temporal coh., # object
for trl, mix in enumerate(trl_ID):
    stim_info[trl, 0] = mix % len(spat_cong)
    stim_info[trl, 1] = (mix // len(spat_cong)) % len(temp_coh)
    stim_info[trl, 2] = ((mix // len(spat_cong)) // len(temp_coh)) % len(n_objects)

with open('exp_info.json', 'w') as outfile:
    json.dump(stim_info.tolist(), outfile)
...
with ExperimentController('test_unity', version='dev', 
    ...
    # %% Tell Unity to load and parse all the trial data
    ec.wait_secs(1) # give stim_info time to save
    client.send_message('/Load_all_data', [0])
